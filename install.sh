...
bucket = "$ARVAN_BUCKET"
endpoint = "$ARVAN_ENDPOINT".replace("https://","")
region = "auto"
service = "s3"
key = "$key"
file_path = "$file"
...
canonical_querystring = f"list-type=2&prefix={prefix}"
...
PY
    ...
    KEYS=$(echo "$LIST" | grep -oP '(?<=<Key>).*?(?=</Key>)' | sort)
...
cmd_install() {
  ...
  (
    crontab -l 2>/dev/null | grep -v "$BIN_PATH backup"
...
cmd_reinstall() {
  ...
  crontab -l 2>/dev/null | grep -v "$BIN_PATH backup" | crontab -
